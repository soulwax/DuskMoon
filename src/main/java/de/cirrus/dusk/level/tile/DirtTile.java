...
public class DirtTile extends Tile {
    ...
    @Override
        ...
        if (!u && !l) {
            screen.render(x * 32, y * 32, Art.i.tileset[0][1], 0);
        } else
            screen.render(x * 32 + 0, y * 32 + 0, Art.i.tileset[l ? 0 : 1][u ? 3 : 4], 0);

        if (!u && !r) {
            screen.render(x * 32 + 16, y * 32 + 0, Art.i.tileset[0][1], 0);
        } else
            screen.render(x * 32 + 16, y * 32 + 0, Art.i.tileset[r ? 2 : 1][u ? 3 : 4], 0);

        if (!d && !l) {
            screen.render(x * 32 + 0, y * 32 + 16, Art.i.tileset[0][1], 0);
        } else
            screen.render(x * 32 + 0, y * 32 + 16, Art.i.tileset[l ? 0 : 1][d ? 5 : 4], 0);
        if (!d && !r) {
            screen.render(x * 32 + 16, y * 32 + 16, Art.i.tileset[0][1], 0);
        } else
